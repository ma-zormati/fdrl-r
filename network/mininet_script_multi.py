...
def testNetwork():
    net = Mininet(topo=None, build=False, ipBase='10.0.0.0/8', autoStaticArp=False)

    # Adding controllers
    info('*** Adding controllers\n')
    c0 = net.addController(name='c0', controller=RemoteController, ip='127.0.0.1', port=9000)
    c1 = net.addController(name='c1', controller=RemoteController, ip='127.0.0.1', port=9001)
    c2 = net.addController(name='c2', controller=RemoteController, ip='127.0.0.1', port=9002)
    c3 = net.addController(name='c3', controller=RemoteController, ip='127.0.0.1', port=9003)
    ...
    # Starting network and controllers
    ...
    net.get('sw5').start([c1])
    net.get('sw6').start([c1])
    net.get('sw7').start([c1])
    net.get('sw8').start([c1])
    net.get('sw9').start([c2])
    net.get('sw10').start([c2])
    net.get('sw11').start([c2])
    net.get('sw12').start([c2])
    net.get('sw13').start([c3])
    net.get('sw14').start([c3])
    net.get('sw15').start([c3])
    net.get('sw16').start([c3])
